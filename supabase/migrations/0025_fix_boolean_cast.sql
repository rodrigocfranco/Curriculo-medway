-- 0025_fix_boolean_cast.sql
-- Fix: evaluate_formula falha com "invalid input syntax for type numeric: true"
-- quando campos boolean (projeto_rondon, etc.) são lidos por operadores que esperam número.
-- Solução: helper safe_numeric que retorna 0 para valores não-numéricos.

-- Helper: safe cast para numeric (retorna 0 se não for numérico)
...
BEGIN
  ...
  IF val = 'true' OR val = 'false' THEN RETURN 0; END IF;
  RETURN val::numeric;
...
-- Recriar evaluate_formula usando safe_numeric para todas as conversões
...
BEGIN
  ...
  WHEN 'sum' THEN
    ...
    LOOP
      ...
      -- override_by_field + override_value (string match)
  ...
  WHEN 'threshold', 'tiered' THEN
    ...
    IF v_aggregate IS NOT NULL AND v_aggregate ? 'sum_of' THEN
      ...
      LOOP
        v_value := v_value + public.safe_numeric(p_data->>v_agg_field);
      END LOOP;
    ELSE
      v_value := public.safe_numeric(p_data->>v_field);
    END IF;
  ...
  WHEN 'ruf_branch' THEN
    ...
    IF p_data ? v_field AND p_data->>v_field IS NOT NULL THEN
      ...
      v_str_value := p_data->>v_field;
      IF v_str_value = 'true' THEN
        v_result := coalesce((p_formula->>'pts_true')::numeric, 0);
      ELSIF v_str_value = 'false' THEN
  ...
  WHEN 'any_positive' THEN
    ...
    LOOP
      IF public.safe_numeric(p_data->>v_field_item) > 0 THEN
        v_found := true;
        EXIT;
      END IF;
    END LOOP;
