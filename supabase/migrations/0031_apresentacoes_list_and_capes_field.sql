-- 0031_apresentacoes_list_and_capes_field.sql
-- 1. Novo campo: faculdade_pos_grad_capes (select)
-- 2. Novo campo: apresentacoes (lista dinâmica tipo + nível)
-- 3. Novo operador: count_tiered (conta itens array → aplica faixas)
-- 4. Migrar apresentacao_congresso → array apresentacoes
-- 5. Atualizar fórmulas de TODAS as instituições

-- =============================================================================
-- 1. Novo campo: faculdade_pos_grad_capes
-- =============================================================================

INSERT INTO curriculum_fields (field_key, label, category, field_type, options, display_order)
VALUES (
  'faculdade_pos_grad_capes',
  'Pós-graduação CAPES da faculdade',
  'Perfil',
  'select',
  '["Doutorado stricto sensu", "Mestrado stricto sensu", "Não possui"]'::jsonb,
  41
)
...
-- =============================================================================
-- 2. Novo campo: apresentacoes (lista dinâmica)
-- =============================================================================
...
VALUES (
  ...
  '{
    ...
    "nivel": ["Congresso de Sociedade Médica (nacional/internacional)", "Congresso acadêmico local/regional"]
  }'::jsonb,
  55
)
ON CONFLICT (field_key) DO UPDATE
...
-- =============================================================================
-- 3. Novo operador: count_tiered
-- Conta itens de array (com filtro opcional) e aplica faixas
-- Exemplo: {"op":"count_tiered","field":"apresentacoes","tiers":[{"gte":3,"pts":10}]}
-- =============================================================================
...
BEGIN
  ...
  -- =========================================================================
  -- COUNT_TIERED: conta itens de array (com filtro opcional) e aplica faixas
  -- {"op":"count_tiered","field":"apresentacoes","tiers":[{"gte":3,"pts":10}]}
  -- =========================================================================
...
-- =============================================================================
-- 4. Migrar dados: apresentacao_congresso numérico → array apresentacoes
-- =============================================================================

UPDATE user_curriculum
SET data = data || jsonb_build_object('apresentacoes', (
  SELECT coalesce(jsonb_agg(
    jsonb_build_object(
      'tipo', 'Apresentação oral',
      'nivel', 'Congresso de Sociedade Médica (nacional/internacional)'
    )
  ), '[]'::jsonb)
  FROM generate_series(1, (data->>'apresentacao_congresso')::int)
))
WHERE coalesce((data->>'apresentacao_congresso')::int, 0) > 0
  AND (NOT data ? 'apresentacoes' OR jsonb_array_length(data->'apresentacoes') = 0);

-- Remover campo antigo do JSONB
UPDATE user_curriculum
SET data = data - 'apresentacao_congresso'
WHERE data ? 'apresentacao_congresso';

-- =============================================================================
-- 5. Atualizar fórmulas — cada instituição
-- =============================================================================

-- SES-PE: 2.5pts por trabalho, max 10
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 2.5, "caps": {"total": 10}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'SES-PE');

-- SES-DF: 0.5 por apresentação, max 1
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 0.5, "caps": {"total": 1}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'SES-DF');

-- Santa Casa SP: faixas por quantidade (1=2.5, 2=5, 3+=10)
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_tiered", "field": "apresentacoes", "tiers": [{"gte": 3, "pts": 10}, {"gte": 2, "pts": 5}, {"gte": 1, "pts": 2.5}]}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'Santa Casa SP');

-- UNICAMP: placeholder — será reescrito na migration 0032
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 2.5, "caps": {"total": 10}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
...
-- USP-RP: 3.3pts cada, max 6
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 3.3, "caps": {"total": 6}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'USP-RP');

-- USP-SP: 5pts cada, max 10
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 5, "caps": {"total": 10}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'USP-SP');

-- Santa Casa BH: threshold ≥1 = 0.25
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 0.25, "caps": {"total": 0.25}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'Santa Casa BH');

-- FMABC: 5pts cada, max 10
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 5, "caps": {"total": 10}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
...
-- PSU-MG: fórmula composta — remover apresentacao_congresso do sum, adicionar regra separada
-- Atualizar regra existente removendo o termo de apresentacao_congresso
UPDATE scoring_rules
SET formula = '{"op": "sum", "caps": {"total": 0.5}, "terms": [{"field": "organizador_evento", "when_gt0": 0.3}, {"field": "ouvinte_congresso", "when_gt0": 0.2}]}'::jsonb,
    max_points = 0.5
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'PSU-MG');

-- PSU-MG: nova regra separada para apresentacoes
INSERT INTO scoring_rules (institution_id, category, field_key, weight, max_points, description, formula)
SELECT id, 'Liderança/Eventos', 'apresentacoes', 0.3, 0.3,
  'Apresentação de trabalho em evento científico (0,3pt)',
  '{"op": "count_articles", "field": "apresentacoes", "mult": 0.3, "caps": {"total": 0.3}}'::jsonb
FROM institutions WHERE short_name = 'PSU-MG';

-- PSU-GO: 0.25pts cada, max 1
UPDATE scoring_rules
SET field_key = 'apresentacoes',
    formula = '{"op": "count_articles", "field": "apresentacoes", "mult": 0.25, "caps": {"total": 1.0}}'::jsonb
WHERE field_key = 'apresentacao_congresso'
  AND institution_id = (SELECT id FROM institutions WHERE short_name = 'PSU-GO');

-- =============================================================================
-- 6. Limpar campo antigo e marcar scores stale
-- =============================================================================

DELETE FROM curriculum_fields WHERE field_key = 'apresentacao_congresso';
