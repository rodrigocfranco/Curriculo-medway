...
- [ ] **Task 1 — Zod schemas `forgotPasswordFormSchema` + `resetPasswordFormSchema`** (AC: #1, #2)
  - [ ] 1.1 Criar `src/lib/schemas/forgot-password.ts`:
    - `email: z.string().trim().toLowerCase().email("Email inválido")`
    - Export `type ForgotPasswordFormValues = z.infer<typeof forgotPasswordFormSchema>`.
  - [ ] 1.2 Criar `src/lib/schemas/reset-password.ts`:
  ...
  - [ ] 1.3 **Testes** co-localizados (`.test.ts`):
  ...
  - [ ] Files: `src/lib/schemas/forgot-password.ts`, `src/lib/schemas/forgot-password.test.ts`, `src/lib/schemas/reset-password.ts`, `src/lib/schemas/reset-password.test.ts`.

- [ ] **Task 2 — Hooks `useRequestPasswordReset` + `useResetPassword` em `src/lib/queries/auth.ts` (estender arquivo existente)** (AC: #1, #2)
  - [ ] 2.1 Abrir `src/lib/queries/auth.ts` (criado em 1.5, estendido em 1.6). **Adicionar** os hooks abaixo mantendo `useSignup`/`useLogin`/`useLogout`/`useCurrentProfile` + mappers existentes intactos.
  - [ ] 2.2 Classe `ResetPasswordError extends Error` com `field: "password" | "confirmPassword" | null` + `message: string`.
  - [ ] 2.3 `mapResetPasswordError(error: AuthError | Error): ResetPasswordError`:
  ...
  - [ ] 2.4 **Hook `useRequestPasswordReset()`** — `useMutation<void, Error, { email: string }>`:
    ```ts
  ...
  - [ ] 2.5 **Hook `useResetPassword()`** — `useMutation<void, ResetPasswordError, ResetPasswordFormValues>`:
    ```ts
    ...
    ```
  - [ ] 2.6 **Teste:** estender `src/lib/queries/auth.test.ts`:
  ...
  - [ ] Files: `src/lib/queries/auth.ts` (estendido), `src/lib/queries/auth.test.ts` (estendido).

- [ ] **Task 3 — `AuthProvider` deve reconhecer `PASSWORD_RECOVERY` sem redirecionar para `/app`** (AC: #2)
  - [ ] 3.1 Editar `src/contexts/AuthContext.tsx` (criado em Story 1.6) — adicionar tratamento do evento `PASSWORD_RECOVERY` no `onAuthStateChange`:
    ```ts
    ...
    ```
  - [ ] 3.2 Expandir `AuthContextValue` com `recoveryMode: boolean`:
    ```ts
    ...
    ```
  - [ ] 3.3 **CRITICAL:** Ajustar `src/pages/app/Home.tsx` (stub da 1.6) e outras páginas que redirecionam via `useAuth()` para **NÃO** redirecionar para `/app` quando `recoveryMode === true`. A página `/reset-password` é a única válida nesse estado. Adicionar guard:
    ```ts
    useEffect(() => {
  ...
  - [ ] 3.4 **Teste:** estender `src/contexts/AuthContext.test.tsx`:
  ...
  - [ ] Files: `src/contexts/AuthContext.tsx` (modificado), `src/contexts/AuthContext.test.tsx` (estendido), `src/pages/app/Home.tsx` (guard), `src/pages/admin/Home.tsx` (guard).

- [ ] **Task 4 — `ForgotPasswordForm` component** (AC: #1)
  - [ ] 4.1 Criar `src/components/features/auth/ForgotPasswordForm.tsx` com `useForm<ForgotPasswordFormValues>({ resolver: zodResolver(forgotPasswordFormSchema), mode: "onSubmit", reValidateMode: "onChange", defaultValues: { email: "" } })`.
  - [ ] 4.2 **Layout** (padrão de `SignupForm` 1.5 + `LoginForm` 1.6):
  ...
  - [ ] 4.3 **Submit handler** (anti-enumeração — mensagem neutra sempre):
    ```ts
    ...
    ```
  - [ ] 4.4 **UI pós-submit:** após `isSuccess || mutation.isError` (qualquer resposta da chamada), renderizar bloco neutro substituindo o formulário:
    ```tsx
    {submittedEmail ? (
  ...
  - [ ] 4.5 **Validação inline âmbar:** `FormMessage` com `className="text-warning"` (ou `text-amber-600` conforme escolha de 1.5). Consistência com Signup/Login.
  - [ ] 4.6 **Acessibilidade:** label acima do input; `aria-invalid` via `FormMessage`; enter submete; sem focus-trap.
  - [ ] Files: `src/components/features/auth/ForgotPasswordForm.tsx`, `src/components/features/auth/ForgotPasswordForm.test.tsx`.

- [ ] **Task 5 — `ResetPasswordForm` component** (AC: #2)
  - [ ] 5.1 Criar `src/components/features/auth/ResetPasswordForm.tsx` com `useForm<ResetPasswordFormValues>({ resolver: zodResolver(resetPasswordFormSchema), mode: "onSubmit", reValidateMode: "onChange", defaultValues: { password: "", confirmPassword: "" } })`.
  - [ ] 5.2 **Layout:**
  ...
  - [ ] 5.3 **Submit handler:**
    ...
    ```
  - [ ] 5.4 **Validação inline âmbar** — mesma regra das outras forms. `FormMessage` texto âmbar.
  - [ ] 5.5 **Acessibilidade:** labels acima; `autoComplete="new-password"` em ambos os campos (password manager oferece salvar); enter submete.
  - [ ] Files: `src/components/features/auth/ResetPasswordForm.tsx`, `src/components/features/auth/ResetPasswordForm.test.tsx`.

- [ ] **Task 6 — Páginas `/forgot-password` e `/reset-password` + rotas** (AC: #1, #2)
  - [ ] 6.1 Criar `src/pages/auth/ForgotPassword.tsx` — mesmo shell público de [Signup.tsx](../../src/pages/auth/Signup.tsx) / Login.tsx (1.6):
    ```tsx
    ...
    ```
  - [ ] 6.2 Criar `src/pages/auth/ResetPassword.tsx` com guard de sessão:
    ```tsx
    const ResetPassword = () => {
  ...
  - [ ] 6.3 Editar `src/router.tsx` — adicionar 2 children da route `AppProviders` antes do `catch-all`, seguindo o padrão path-relativo lazy de 1.5 e 1.6:
    ```ts
    {
  ...
  - [ ] 6.4 Files: `src/pages/auth/ForgotPassword.tsx`, `src/pages/auth/ForgotPassword.test.tsx`, `src/pages/auth/ResetPassword.tsx`, `src/pages/auth/ResetPassword.test.tsx`, `src/router.tsx` (modificado).

- [ ] **Task 7 — Configuração Supabase: URL de redirect autorizada** (AC: #2)
  - [ ] 7.1 Editar `supabase/config.toml` local — garantir que `[auth]` tenha `additional_redirect_urls` incluindo:
  ...
  - [ ] 7.2 Verificar `site_url` — deve apontar para URL do ambiente. Local = `http://localhost:8080` (mesmo padrão da 1.5). Produção = URL de deploy (Vercel/Netlify — Story 1.11).
  - [ ] 7.3 **Smoke:** `supabase stop && supabase start` para recarregar config → testar envio de email via Inbucket (`http://127.0.0.1:54324`) — link deve apontar para `http://localhost:8080/reset-password?code=...`. Se apontar para URL diferente, reset falha silenciosamente (Supabase bloqueia redirect não autorizado) — **este é o failure-mode mais comum**, **revisar config primeiro** ao debugar.
  - [ ] 7.4 Documentar em [deferred-work.md](./deferred-work.md) entry nova: "Redirect URLs de produção precisam ser adicionadas ao Supabase Dashboard antes do primeiro deploy (Story 1.11)".
  - [ ] Files: `supabase/config.toml` (modificado), `_bmad-output/implementation-artifacts/deferred-work.md` (append).

- [ ] **Task 8 — Testes de integração** (AC: #1, #2)
  - [ ] 8.1 `ForgotPasswordForm.test.tsx`:
  ...
  - [ ] 8.2 `ResetPasswordForm.test.tsx`:
  ...
  - [ ] 8.3 `ForgotPassword.test.tsx` (page):
    - Render header + h1 "Recuperar senha" + `ForgotPasswordForm`.
  - [ ] 8.4 `ResetPassword.test.tsx` (page):
  ...
  - [ ] 8.5 `AuthContext.test.tsx` extensão — já coberto em Task 3.4.
  - [ ] 8.6 Usar `@testing-library/react` + `@testing-library/user-event` + `vi.mock("@/lib/supabase")` + `vi.mock("sonner")`. **Não** criar mock global — segue padrão de 1.5 e 1.6.

- [ ] **Task 9 — Acessibilidade + QA manual** (AC: #1, #2)
  - [ ] 9.1 Keyboard-only: Tab percorre email → CTA → link "Voltar"; Enter em input submete. Sem focus-trap.
  - [ ] 9.2 Autocomplete: email `autoComplete="email"` + `inputMode="email"`; `ResetPasswordForm` usa `autoComplete="new-password"` em ambos (instrui password manager a **salvar** nova senha — não auto-fill de senha antiga).
  - [ ] 9.3 **Sem PII em logs** — `mapResetPasswordError` não loga password/email. Remover qualquer `console.log` de debug antes do PR.
  - [ ] 9.4 `bun run lint` + `bun run test` + `bun run build` devem passar 100%. **Não** introduzir warnings novos além do baseline.
  - [ ] 9.5 **Smoke manual completo** (requer `supabase start` + `bun dev` + Inbucket em `http://127.0.0.1:54324`):
...
- [ ] **Task 10 — Atualizar `LoginForm` (opcional polimento)** (AC: —)
  - [ ] 10.1 Verificar que o link `"Esqueci minha senha"` em `src/components/features/auth/LoginForm.tsx` (criado em 1.6) aponta para `/forgot-password` — se aponta, nada a fazer. Se por qualquer motivo estiver apontando para outra coisa (regressão), corrigir.
  - [ ] 10.2 **Não** mover o link para dentro de `FormMessage` — fica abaixo do CTA "Entrar" conforme decisão de 1.6.
  - [ ] Files: `src/components/features/auth/LoginForm.tsx` (só se necessário — zero-diff esperado).

- [ ] **Task 11 — Update sprint-status + deferred-work** (AC: —)
  - [ ] 11.1 Após dev-story completar e code-review passar, `sprint-status.yaml` será atualizado (ready-for-dev → in-progress → review → done) pelos workflows subsequentes.
  - [ ] 11.2 **Deferred esperados** (anotar em `deferred-work.md` na fase code-review):
