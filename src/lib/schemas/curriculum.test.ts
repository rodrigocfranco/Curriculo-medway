...
describe("curriculumDataSchema", () => {
  it("aceita dados válidos completos", () => {
    const result = curriculumDataSchema.safeParse({
      ...
      cursos_suporte: 3,
      apresentacoes: [
        { tipo: "Apresentação oral", nivel: "Congresso de Sociedade Médica", abrangencia: "Nacional" },
        { tipo: "Poster", nivel: "Congresso acadêmico", abrangencia: "Regional" },
      ],
      ouvinte_congresso: 5,
  ...
  it("valida todos os campos conhecidos com defaults", () => {
    ...
    if (result.success) {
      const knownFields = [
        ...
        "ranking_ruf_top35",
        "faculdade_tem_doutorado", "faculdade_tem_mestrado", "faculdade_programas_capes",
